...
typedef struct SNDSTRUC_typ
        ...
        unsigned char *sound;   // a pointer to the raw sound data
        unsigned short sndlen;      // the length of the sound data in bytes
        short *IsPlaying;       // a pointer to a variable that will be
...
typedef struct sound_typ
        {
        unsigned char *buffer;   // pointer to the start of VOC file
...
typedef struct music_typ
        ...
        unsigned char *buffer;   // pointer to midi data
...
int Sound_Load(char *filename, sound_ptr sound,int translate);
