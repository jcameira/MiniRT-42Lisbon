...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/17 15:34:45 by jcameira          #+#    #+#             */
/*   Updated: 2025/04/17 15:35:15 by jcameira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	alloc_lights(t_list **new_o, t_list **new_l, t_object **o_content,
	t_light **l_content)
{
	(*new_l) = new_light();
	if (!(*new_l))
	...
	(*new_o) = new_object();
	if (!(*new_o))
		return (ft_dprintf(2, NO_SPACE), free((*new_l)), 0);
...
int	parse_default_light(t_scene *scene, char *line)
{
	...
	if (!alloc_lights(&new_o, &new_l, &o_content, &l_content))
		return (free_arr((void **)info), 0);
	if (!parse_light_info(scene, new_o, o_content, info))
		return (ft_dprintf(2, LIGHT_USAGE), free(l_content), free(o_content),
			free_arr((void **)info), free(new_o), free(new_l), 0);
