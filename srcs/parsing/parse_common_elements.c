...
int	parse_cam(t_scene *scene, char *line)
{
	...
	if (arr_size(line_info) != 4)
		return (ft_dprintf(2, CAMERA_USAGE), free_arr((void **)line_info), 0);
	if (!parse_point(&scene->cam.o, line_info[1], 0))
		return (ft_dprintf(2, CAMERA_USAGE), free_arr((void **)line_info), 0);
	if (!parse_point(&scene->cam.nv, line_info[2], 0))
		return (ft_dprintf(2, CAMERA_USAGE), free_arr((void **)line_info), 0);
	if (!check_if_float(line_info[3]))
		return (ft_dprintf(2, CAMERA_USAGE), free_arr((void **)line_info), 0);
	scene->cam.fov = ft_atoi(line_info[3]);
	if (!in_range((float)scene->cam.fov, (float)FOV_MIN, (float)FOV_MAX))
		return (ft_dprintf(2, FOV_ERROR), free_arr((void **)line_info), 0);
...
int	parse_ambience(t_scene *scene, char *line)
{
	...
	if (!line_info)
		return (ft_dprintf(2, NO_SPACE), 0);
	if (arr_size(line_info) != 3)
		return (ft_dprintf(2, AMBIENCE_USAGE), free_arr((void **)line_info), 0);
	if (!check_if_float(line_info[1]))
		return (ft_dprintf(2, AMBIENCE_USAGE), free_arr((void **)line_info), 0);
	scene->amb.al_br = ft_atof(line_info[1]);
	if (!in_range(scene->amb.al_br, BR_MIN, BR_MAX))
		return (ft_dprintf(2, AMBIENCE_USAGE), free_arr((void **)line_info), 0);
	if (!parse_color(&scene->amb.al_c, line_info[2]))
		return (ft_dprintf(2, AMBIENCE_USAGE), free_arr((void **)line_info), 0);
	scene->amb.has_al = 1;
	return (free_arr((void **)line_info), 1);
}
...
int	parse_default_light(t_scene *scene, char *line)
{
	...
	if (!line_info)
		return (ft_dprintf(2, NO_SPACE), 0);
	if (arr_size(line_info) != 4)
		return (ft_dprintf(2, LIGHT_USAGE), free_arr((void **)line_info), 0);
	new_o = new_object();
	if (!new_o)
		return (ft_dprintf(2, NO_SPACE), 0);
	new_l = new_light();
	...
	if (!parse_point(&o_content->sp.c, line_info[1], 0))
